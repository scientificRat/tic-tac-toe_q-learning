...
int main(int argc, const char * argv[]) {
    ...
    Player *p_random=new RandomMachine("player_r");
    ConsoleDisplay* display=new ConsoleDisplay('X','O');
    GameManager *m=GameManager::create(display,gameBoard, 3, p_machine1, p_minimax2);
    m->runGameWithoutDisplay(10000);
    //m->setPlayer(p_machine1, p_minimax2);
    //m->runGameWithoutDisplay(1000);
    //m->setPlayer(p_minimax1, p_machine2);
    //m->runGameWithoutDisplay(1000);
    m->setPlayer(p_machine1, p2);
    m->setPlayer(p_minimax1, p2);
//    m->runGame(1000);
    delete gameBoard;
    delete p1;
    delete p2;
